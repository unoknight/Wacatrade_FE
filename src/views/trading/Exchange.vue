        ...
        <div class="vx-row">
            <div class="w-full">
                <div class="exchange-currency">
                    <h4 class="exchange-currency-title color-white font-bold mb-3 pt-4 capitalize">{{ $t(" Exchange_TransferMoney")}}</h4>
                    <div class="vx-row boxExchange">
                        <div class="vx-col w-full lg:w-4/12">
                            <div class="send">
                                <div class="inputAmount mt-0 mb-1">
                                    <div class="flex justify-between items-center"><span class="font-bold font-12-m color-white">{{ $t(" Exchange_Send")}}</span>
                                        <div class="inputLabel absolute right-0">
                                            <a href="javascript:;" class="butt text-center" @click="ttClickAmount(1)">{{ $t(" Exchange_Part")}}</a> 
                                            <a href="javascript:;" class="butt text-center" @click="ttClickAmount(2)">{{ $t(" Exchange_All")}}</a>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="boxSelectUnit">
                                        <div :class="`selectedUnit ${isCoinOne}`">
                                            ...
                                            <div class="info"><span class="titleUnit"></span>
                                                <div class="amount"><span class="label">{{ $t(" Exchange_YourBalance")}}</span> <span class="number px-1">
                                                            {{ balanceOne }}
                        ...
                        <div class="vx-col w-full lg:w-4/12">
                            <div class="receive">
                                <span class="inputAmount font-bold mt-0 mb-1 inline-block color-white">{{ $t("Exchange_Receive1")}}</span>
                    ...
                    <div class="processExchange"><span class="font-12 color-white">{{ $t(" Exchange_MinTransfer")}} {{ minAmountTrans + ' ' + isCoinOne }}</span>
                    </div>
                </div>
                <div class="historyEx lg:mb-4">
                    <h4 class="mb-3 history-title">{{ $t(" Exchange_HistoryTransfer")}}</h4>
                    <div class="history-body relative" :class="{'ld-loading': isLoading}">
                        <table class="table listHis ex w-full">
                            ...
                            <thead class="header">
                                <tr>
                                    <th>{{ $t(" Exchange_Time")}}</th>
                                    <th>{{ $t(" Exchange_Send")}}</th>
                                    <th>{{ $t(" Exchange_Receive")}}</th>
                                    <th>{{ $t(" Exchange_From")}}</th>
                                    <th>{{ $t(" Exchange_To")}}</th>
                                </tr>
                            </thead>
...
components: {
  ...
  data () {
    return {
        ...
        ssDownSend: this.$t('Exchange_Transfer'),
      ...
      sendCodeSecure(){
        ...
        let countD = setInterval(() => {
            ...
            if(giay == 0){
                ...
                this.ssDownSend = this.$t('Exchange_Transfer')
    ...
    exChangeSubmit(){
          ...
          if(am < adf){
              return this.$vs.notify({
                    text: this.$t('Exchange_MinTransfer')` ${adf} ${this.isCoinOne}`,
                    color:'danger',
                    position:'top-right',
                    iconPack: 'feather',
                    icon:'icon-x-circle'
          ...
          if(am > dfBalance){
              return this.$vs.notify({
                    text: this.$t('Exchange_NotEnoughBalance'),
                    color:'danger',
                    position:'top-right',
                    iconPack: 'feather',
                    icon:'icon-x-circle'
            ...
            .then((res) => {
                ...
                if(d.success == 1){
                    ...
                    return this.$vs.notify({
                        text:this.$t('Exchange_TransferSuccess'),
                        color:'success',
                        position:'top-right',
                        iconPack: 'feather',
                        icon:'icon-check'
