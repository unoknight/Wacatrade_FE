    ...
    <VuePerfectScrollbar style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);" v-if="dataType != 'addMoney'" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <template v-if="dataImg">
          ...
          <!-- Image Container -->
          <div class="flex items-center justify-center w-64 mx-auto img-container">
            <img :src="`${dm}api/auth/me/photo/${dataImg}`" width="100" alt="img">
          </div>

          <!-- Image upload Buttons -->
          <!--<div class="flex justify-between mt-5 modify-img">
        ...
        <!-- EMAIL -->
        <vs-input label="Hộp thư" v-model="dataEmail" class="w-full mt-5" type="email" name="item-email" v-validate="'required|email|min:6|max:200'" />
        <span class="text-sm text-danger" v-show="errors.has('item-email')">{{ errors.first('item-email') }}</span>

        <!-- NICKNAME -->
        <vs-input label="Biệt danh" v-model="dataNick" class="w-full mt-5" name="item-nick" v-validate="'required|min:6|max:20'" />
        <span class="text-sm text-danger" v-show="errors.has('item-nick')">{{ errors.first('item-nick') }}</span>


        <!-- NAME -->
        <vs-input label="Họ" v-model="dataFirstName" class="w-full mt-5" name="item-first_name" />

        <vs-input label="Tên" v-model="dataLastName" class="w-full mt-5" name="item-last_name" />
        ...
        <span class="text-sm text-danger" v-show="errors.has('item-wallet_sys')">{{ errors.first('item-wallet_sys') }}</span>
        ...
        <span class="text-sm text-danger" v-show="errors.has('item-wallet_btc')">{{ errors.first('item-wallet_btc') }}</span>
        ...
        <span class="text-sm text-danger" v-show="errors.has('item-wallet_eth')">{{ errors.first('item-wallet_eth') }}</span>
        ...
        <span class="text-sm text-danger" v-show="errors.has('item-wallet_usdt')">{{ errors.first('item-wallet_usdt') }}</span>
        -->
        ...
        <template v-else>
            <vs-select v-model="dataPermission" label="Quyền" class="w-full mt-5" name="item-category" v-validate="'required'">
            <vs-select-item :key="item.value" :value="item.value" :text="item.text" v-for="item in category_choices" />
            </vs-select>
            <span class="text-sm text-danger" v-show="errors.has('item-category')">{{ errors.first('item-category') }}</span>

            <vs-select v-if="dataPermission" v-model="isLevelVIP" label="Cấp" class="w-full mt-5" name="item-category" v-validate="'required'">
        ...
        <!-- ORDER STATUS -->
        <!-- <vs-select v-model="dataOrder_status" label="Trạng thái 2FA" class="w-full mt-5">
        ...
        <!-- <vs-input
          ...
          v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }"
          name="item-price" />
        <span class="text-sm text-danger" v-show="errors.has('item-price')">{{ errors.first('item-price') }}</span> -->

        <!-- Upload -->
        <!-- <vs-upload text="Upload Image" class="img-upload" ref="fileUpload" /> -->

        <!-- <div class="mt-5 upload-img" v-if="!dataImg">
    ...
    <VuePerfectScrollbar v-else  style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6" >
          ...
          <vs-input 
              ...
              v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }"
              name="item-wallet_vn" />
          <span class="text-sm text-danger" v-show="errors.has('item-wallet_vn')">{{ errors.first('item-wallet_vn') }}</span>
