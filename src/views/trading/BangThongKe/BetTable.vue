...
<template>
  <div id="list-bet-trade" class="data-list-container">
    ...
    <div class="flex-between-center">
      ...
      <p style="color: rgb(25 146 246)">{{ $t("BetTable_TotalWin")}}{{ this.formatPrice(betWin, 2) }}</p>
      <p style="color: #f00">{{ $t("BetTable_TotalLost")}}{{ this.formatPrice(betLose, 2) }}</p>
    </div>
    <div class="flex items-end">
      <div class="mr-4">
        <small class="date-label">{{ $t("BetTable_From")}}</small>
        <datepicker v-model="startDate" format="dd-MM-yyyy"></datepicker>
      </div>
      <div class="mr-4">
        <small class="date-label">{{ $t("BetTable_To")}}</small>
        <datepicker
          name="end-date"
          v-model="endDate"
          format="dd-MM-yyyy"
    ...
    <div id="loading-corners-bet" class="vs-con-loading__container">
      ...
      >
        ...
        <template slot="thead">
          <vs-th sort-key="description">{{ $t("BetTable_Des")}}</vs-th>
          <vs-th sort-key="type">{{ $t("BetTable_Type")}}</vs-th>
          <vs-th sort-key="buy_sell">{{ $t("BetTable_BuySell")}}</vs-th>
          <vs-th sort-key="amount">{{ $t("BetTable_BetMoney")}}</vs-th>
          <vs-th sort-key="win_lose">{{ $t("BetTable_WinLost")}}</vs-th>
          <vs-th sort-key="result_bet">{{ $t("BetTable_Result")}}</vs-th>
          <vs-th sort-key="open_close">{{ $t("BetTable_OpenClose")}}</vs-th>
          <vs-th sort-key="datecreate">{{ $t("BetTable_Time")}}</vs-th>
          <!--<vs-th>Tác vụ</vs-th>-->
        </template>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="bet-currency">
                  ...
                  <span
                    ...
                    >Marketing</span
                  >
                  <span v-else>{{ $t("BetTable_Normal")}}</span>
                </p>
              </vs-td>

              <vs-td>
                <p class="bet-buy_sell" v-if="tr.buy_sell == 'buy'">
                  <span class="text-success">{{ $t("BetTable_Buy")}}</span>
                </p>
                <p class="bet-buy_sell" v-else>
                  <span class="text-danger">{{ $t("BetTable_Sell")}}</span>
                </p>
              </vs-td>
              ...
              <vs-td>
                <p class="bet-result_bet" v-if="tr.amount_win != 0">
                  <span class="text-success">{{ $t("BetTable_WIN")}}</span>
                </p>
                <p class="bet-result_bet" v-else>
                  <span class="text-danger">{{ $t("BetTable_LOST")}}</span>
                </p>
              </vs-td>

              <vs-td>
                <p class="bet-open_close">{{ `${tr.open} -> ${tr.close}` }}</p>
                <p>{{ $t("BetTabble_Phien")}} {{ tr.session }}</p>
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    getOrderStatusColorText(status) {
      if (status == 0) return this.$t('BetTabble_WaitProcess');
      if (status == 1) return this.$t('F17NapTable_Done');
      //if(status == 2) return "danger"
      return this.$t('BetTabble_WaitProcess');
    },
