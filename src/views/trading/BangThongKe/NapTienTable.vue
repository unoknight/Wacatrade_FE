...
<template>
  <div id="list-history-deposit" class="data-list-container">
    ...
    <div class="flex-between-center">
      ...
      <p></p>
      <div>{{ $t(" Deposit_TotalTitle")}}</div>
      <p style="color: #00ffad">
        {{ $t(" Deposit_TotalDeposit")}}{{ this.formatPrice(tongNap, 2) }}
      </p>
    </div>

    <div class="flex items-end">
      <div class="mr-4">
        <small class="date-label">{{ $t(" Deposit_From")}}</small>
        <datepicker v-model="startDate" format="dd-MM-yyyy"></datepicker>
      </div>
      <div class="mr-4">
        <small class="date-label">{{ $t(" Deposit_To")}}</small>
        <datepicker
          name="end-date"
          v-model="endDate"
          format="dd-MM-yyyy"
    ...
    <div id="loading-corners-nap" class="vs-con-loading__container">
      ...
      >
        ...
        <template slot="thead">
          <vs-th sort-key="account">{{ $t(" Deposit_Account")}}</vs-th>
          <vs-th sort-key="type">{{ $t(" Deposit_TypeMoney")}}</vs-th>
          <vs-th sort-key="amount">{{ $t(" Deposit_AmountMoney")}}</vs-th>
          <vs-th sort-key="note">{{ $t(" Deposit_Note")}}</vs-th>
          <vs-th sort-key="status">{{ $t(" Deposit_Status")}}</vs-th>
          <vs-th sort-key="datecreate">{{ $t(" Deposit_Time")}}</vs-th>
          <!-- <vs-th>Tác vụ</vs-th> -->
        </template>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              <vs-td>
                <p class="de-name font-medium truncate">
                  {{ tr.from_u }}<br />{{ $t(" Deposit_Des")}} {{ tr.type }}
                </p>
              </vs-td>
