...
<template>
  <div id="list-history-deposit" class="data-list-container">
    <div class="flex-between-center">
      <div>{{ $t("F17NapTable_Total")}} {{tableType == "nap" ? "NẠP" : "RÚT"}} {{ $t("F17NapTable_Success")}}</div>
      <p style="color: #00ffad">
        {{ $t("F17NapTable_Total")}} {{tableType == "nap" ? "nạp" : "rút"}}: ${{ this.formatPrice(tongNap, 2) }}
      </p>
    </div>
    ...
    <div id="loading-corners-nap" class="vs-con-loading__container">
      ...
      >
        ...
        <template slot="thead">
          <vs-th sort-key="account">{{ $t("F17NapTable_Account")}}</vs-th>
          <vs-th sort-key="type">{{ $t("F17NapTable_TypeMoney")}} {{ $t("F17NapTable_Money")}}</vs-th>
          <vs-th sort-key="amount">{{ $t("F17NapTable_AmountMoney")}}</vs-th>
          <vs-th sort-key="note">{{ $t("F17NapTable_Note")}}</vs-th>
          <vs-th sort-key="status">{{ $t("F17NapTable_Status")}}</vs-th>
          <vs-th sort-key="datecreate">{{ $t("F17NapTable_Time")}}</vs-th>
          <!-- <vs-th>Tác vụ</vs-th> -->
        </template>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              <vs-td>
                <p class="de-name font-medium truncate">
                  {{ tr.from_u }}<br />{{ $t("F17NapTable_Des")}} {{ tr.type }}
                </p>
              </vs-td>
      ...
      AuthenticationService.deleteTrashByID(obj).then((resp) => {
        if (resp.data.success) {
          ...
          return this.$vs.notify({
            text: this.$t('F17NapTable_DelDone'),
            color: "success",
            iconPack: "feather",
            icon: "icon-check",
          });
