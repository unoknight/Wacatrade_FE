      ...
      <vs-table ref="table" multiple v-model="selectedUser" pagination :max-items="itemsPerPage" search :data="products">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center justify-between flex-grow">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            <select
              class="block w-full p-3 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
              <option selected value="0">Tài khoản thường</option>
              <option value="1">Tài khoản Marketing</option>
            </select>
          </div>
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data.filter((e) => e.type_account)">
              <vs-td>
                <p class="font-medium truncate bet-description">Email: {{ tr.email }}</p>
                <p class="font-medium truncate bet-description">ID: {{ tr.id_account }}</p>
                <p class="font-medium truncate bet-description" v-if="tr.type_account">Live Account</p>
                <p class="font-medium truncate bet-description" v-else>Demo Account</p>
              </vs-td>
              ...
              <vs-td>
                <p class="bet-win_lose">
                  <span style="color: #fb9494;font-weight: bold;">{{ `${tr.amount_win != 0 ? '+' + tr.amount_win :
                    '-' + tr.amount_lose}` }}</span>
                </p>
              </vs-td>
