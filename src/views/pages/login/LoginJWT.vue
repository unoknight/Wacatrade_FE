<template>
  <div>
    <vs-input
        ...
        icon="icon icon-user"
        icon-pack="feather"
        :label-placeholder="this.$t('CopyTrade_Setting_Account')"
        v-model="username"
        class="w-full"/>
    ...
    <vs-input
        ...
        icon="icon icon-lock"
        icon-pack="feather"
        :label-placeholder="this.$t('Profile_Password')"
        v-model="password"
        class="w-full mt-6" />
    <span class="text-sm text-danger">{{ errors.first('password') }}</span>

    <div class="flex flex-wrap justify-between my-5">
        <vs-checkbox v-model="checkbox_remember_me" class="mb-3">{{ $t("Login_Remember")}}</vs-checkbox>
        <!-- <router-link to="/pages/forgot-password">Quên mật khẩu?</router-link> -->
    </div>
    <div class="flex flex-wrap justify-between mb-3">
      <!--<vs-button  type="border" @click="registerUser">Register</vs-button>-->
      <vs-button :disabled="!validateForm" @click="loginJWT">{{ $t("Login_LoginButton")}}</vs-button>
    </div>
  </div>
</template>
