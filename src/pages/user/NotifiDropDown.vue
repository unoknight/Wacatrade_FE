<template>
  <div>
    <vs-dropdown
      vs-custom-content
      vs-trigger-click
      class="cursor-pointer wrapper-notification-1"
    >
      <svg
        class="block w-5 h-5 text-center"
      ...
      <span class="block ttM">{{ $t("Notification") || "Notification" }}</span>
      ...
      <vs-dropdown-menu class="dropdown-custom vx-navbar-dropdown">
        <div class="wrapper-notification">
          ...
          <div class="right" @click="goNotificationPage()">{{ $t("View_all") || "No View_all" }}</div>
        </div>
        <div id="loading-corners" class="vs-con-loading__container">
          <VuePerfectScrollbar
            ref="mainSidebarPs"
            class="p-0 scroll-area--nofications-dropdown"
            :settings="settings"
            :key="$vs.rtl"
          >
            <ul class="bordered-items">
              ...
              >
                ...
                <div>{{ $t("No_Notification") || "No Notification" }}</div>
              </div>
              <li
                ...
                <div class="flex items-center">
                  ...
                  <div class="mx-2">
                    ...
                    <span
                      ...
                      class="block font-medium notification-title"
                      :class="[`text-${ntf.category}`]"
                      >{{ ntf.title_en ? ntf.title_en : ntf.title }}</span
                    >

                    <span
                      v-else-if="locale === 'cam'"
                      class="block font-medium notification-title"
                      :class="[`text-${ntf.category}`]"
                      >{{ ntf.title_cam ? ntf.title_cam : ntf.title }}</span
                    >

                    <small v-if="locale === 'vi'" v-html="ntf.msg"></small>
                    <small v-else-if="locale === 'en'" v-html="ntf.msg_en ? ntf.msg_en : ntf.msg"></small>
                    <small v-else-if="locale === 'cam'" v-html="ntf.msg_cam ? ntf.msg_cam : ntf.msg"></small>
        ...
        if (res.data.success) {
          ...
          for (let i = 0; i < dataN.length; i++) {
            ...
            let json = {
              index: i,
              title: dataN[i].title,
              title_en: dataN[i].title_en ? dataN[i].title_en : '',
              title_cam: dataN[i].title_cam ? dataN[i].title_cam : '',
              msg: dataN[i].content,
              msg_en: dataN[i].content_en ? dataN[i].content_en : '',
              msg_cam: dataN[i].content_cam ? dataN[i].content_cam : '',
              type: dataN[i].type,
          ...
          this.unreadNotifications = [
            {
              ...
              title_cam: data.title_cam ? data.title_cam : '',
              msg: data.content,
              msg_en: data.content_en ? data.content_en : '',
              msg_cam: data.content_cam ? data.content_cam : '',
              type: data.type,
              time: moment().fromNow(),
              category: "success",
              views: 0,
