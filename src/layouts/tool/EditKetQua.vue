      ...
      <div class="vx-row">
        ...
        <div class="w-full vx-col lg:w-2/3">
          ...
          <div>
            ...
            <div>
              <vs-textarea v-model="ACCOUNT_BREAK"  height="120px" />
            </div>
            <vs-button @click="sbAccountBeCau" class="h-12 font-bold" color="success" >Lưu</vs-button>
          </div>
        </div>
          ...
          <table class="tbv">
            ...
            <tr :data="tr" :key="indextr" v-for="(tr, indextr) in productsFake">
              <template v-if="tr.mkt == 1">
                <td>{{ tr.e }}</td>
                <td>
                  <p class="ml-2" color="#006c27">{{ tr.before }}</p>
                </td>
                <td>
                ...
                <td>
                  <p class="bet-amount">
                    ...
                    <font class="ml-2" color="#006c27">{{ formatPrice(tr.amount, 2) }} ({{ formatPrice(tr.percen,2) }}%)</font>
                  </p>
                </td>
              </template>
            ...
            <tr :data="tr" :key="indextr" v-for="(tr, indextr) in productsFake">
              <template v-if="tr.mkt == 0">
                ...
                <td>
                  <p class="bet-amount">
                    <IconCrypto style="width: 20px" coinname="usdt" color="color" format="svg" />
                    <font class="ml-2" color="#006c27">{{ formatPrice(tr.amount, 2) }} ({{ formatPrice(tr.percen,2) }}%)
                    </font>
                  </p>
                </td>
              </template>
  ...
  methods: {
    ...
    sbAccountBeCau(){
      this.sendMessage({
        type: "editGL",
        data: {
          type: "WRITE_ACCOUNT_BREAK",
          accounts: this.ACCOUNT_BREAK,
        },
      });
      return this.$vs.notify({
        text: "Đã thay đổi danh sách email bẻ cầu",
        color: "success",
        position: "top-center",
        iconPack: "feather",
        icon: "icon-message-square",
  ...
  created() {
    ...
    this.connection.onmessage = function (event) {
      ...
      if (data.type === "getTruck") {
        ...
        _this.ACCOUNT_BREAK = data.account_break;
      }

      if (data.type === "truck") {
        _this.productsFake = dl;
      
        _this.productsFake.forEach(element => {
          element.percen = (parseFloat(element.amount) * 100) / parseFloat(element.before);
        });

        _this.productsFake.sort((a, b) => parseFloat(b.percen) - parseFloat(a.percen));

        console.log(_this.productsFake);

        // _this.productsFake.forEach(element => {
        //   element.percen = (element.amount * 100) / element.before;
        // });
