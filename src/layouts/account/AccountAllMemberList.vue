  ...
  <div id="list-all-account" class="data-list-container">
              ...
              <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                  ...
                  <vs-td>
                      ...
                      <p>
                          ...
                          *** HHVIP: <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.amount_hhvip, 2) }}</span><br>
                          *** HHGD: <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.amount_hhgd, 2) }}</span><br>
                          *** Nạp: <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.amount_nap, 2) }}</span><br>
                          *** Rút: <span class="italic font-bold" style="color: #ff5c5c">${{ formatPrice(tr.amount_rut, 2) }}</span><br>
                ...
                <vs-td>
                  ...
                  {{ tr.first_name +' '+ tr.last_name }}<br>
                    ...
                    <vs-checkbox :value="tr.marketing" @change="changeAccType(tr)">Marketing</vs-checkbox>
                  </p>
                  <p>
                ...
                <vs-td>
                  <p class="user-price" @click="scanWalletAdmin(tr)">
    ...
    <vs-popup title="Danh sách thành viên F1 - F7" :active.sync="popupF1F7">
          <div style="display: flex; flex-direction: row;margin-bottom: 10px;">
            <vs-input v-model="startDate" :placeholder="`Start`"></vs-input>
            <vs-input v-model="endDate" :placeholder="`End`"></vs-input>
            <vs-button @click="filerDate">Filter</vs-button>
          </div>
          <div id="loading-corners2" class="vs-con-loading__container">
              <p>
                <span class="mr-2" v-if="this.tslVolFilter > 0" style="color:red;font-size: 20px;">Tổng Volume filter: ${{ formatPrice(tslVolFilter, 2) }}</span><br>
                <span class="mr-2" v-if="this.tslVolFilterCP > 0" style="color:red;font-size: 20px;">Tổng Volume CP filter: ${{ formatPrice(tslVolFilterCP, 2) }}</span><br>
                <span class="mr-2" v-if="this.tslHH > 0" style="color:red;font-size: 20px;">Tổng HH filter: ${{ formatPrice(tslHH, 2) }}</span><br>
                <span class="mr-2" v-if="this.tslHHVip > 0" style="color:red;font-size: 20px;">Tổng HHVip filter: ${{ formatPrice(tslHHVip, 2) }}</span><br>
                <span class="mr-2">Tổng Volumn Giao dịch: ${{ formatPrice(tslgdCD, 2) }}</span><br>
                <span class="mr-2">Tổng HH VIP: ${{ formatPrice(tshhVip, 2) }}</span><br>
                <span class="mr-2">Tổng HH Giao dịch: ${{ formatPrice(tshhTrade, 2) }}</span><br>
                <span class="mr-2">Tổng Nạp: ${{ formatPrice(tslNap, 2) }}</span><br>
                <span class="mr-2">Tổng Rút: ${{ formatPrice(tslRut, 2) }}</span><br>
                <span class="mr-2">Tổng CD Thắng: ${{ formatPrice(tslWin, 2) }}</span><br>
                <span class="mr-2">Tổng CD Thua: ${{ formatPrice(tslLose, 2) }}</span><br>
                <span class="mr-2">Tổng hệ thống lời: ${{ formatPrice(tslLoi, 2) }}</span><br>
                <span class="mr-2">Số dư: ${{ formatPrice(tlsSodu, 2) }}</span><br>
            </p>
...
export default {
  ...
  methods: {
    scanWalletAdmin(tr){
      ...
      AuthenticationService.scanWalletAdmin(tr.email).then((req) =>{
        return this.$vs.notify({
                  text:'Scan tài khoản thành công!',
                  color:'success',
                  iconPack: 'feather',
                  position: 'top-right',
                  icon:'icon-check'});
          ...
          if(resp.data.success){
              ...
              for(let obj in data) {
                ...
                for(let i = 0; i < data[obj].length; i++){
                  let userData = data[obj][i];

                  this.tslgdCD += parseFloat(userData.tklgd);
                  this.tshhVip += parseFloat(userData.commission_vip);
                  this.tshhTrade += parseFloat(userData.pending_commission);
                  this.tslNap += parseFloat(userData.amt);
                  this.tslRut += parseFloat(userData.rut);
                  this.tslWin += parseFloat(userData.priceWin);
                  this.tslLose += parseFloat(userData.priceLose);
                  this.tlsSodu += parseFloat(userData.balance) + parseFloat(userData.money_usdt);
                  
                  let loi = parseFloat(userData.priceLose) - parseFloat(userData.priceWin) - parseFloat(userData.commission_vip) - parseFloat(userData.pending_commission);
                  let sodu = parseFloat(userData.balance) + parseFloat(userData.money_usdt);
                  this.tslLoi += loi;

                  this.treeData[0].children.push({title: `<span style='color: white'>Cấp ${c} - ${data[obj][i].nick_name} <br>- Volume: ${ this.formatPrice(data[obj][i].tklgd, 2)}</span> <br>- Tổng tiền nạp: ${this.formatPrice(data[obj][i].amt, 2)}<br>- Tổng tiền rút: ${this.formatPrice(data[obj][i].rut, 2)} <br>- Tổng tiền thắng: ${this.formatPrice(data[obj][i].priceWin, 2)}  <br>- Tổng tiền thua: ${this.formatPrice(data[obj][i].priceLose, 2)}<br>- Tổng HHVIP: ${this.formatPrice(data[obj][i].commission_vip, 2)}<br>- Tổng HHGD: ${this.formatPrice(data[obj][i].pending_commission, 2)} <br> - Số dư: ${this.formatPrice(sodu, 2)}`})
                }
              }
          }
        })
    ...
    filerDate(){
      ...
      .then((resp) => {
        ...
        if(resp.data.success){
            ...
            let data = resp.data.data;
            let obj1 =  resp.data.obj;
            this.tslgdCD1 = obj1.tslgdCD1;
            this.tslgdCD2 = obj1.tslgdCD2;
            this.tslgdCD3 = obj1.tslgdCD3;
            this.tslgdCD4 = obj1.tslgdCD4;

            this.tslVolFilter = obj1.filterVolumn;
            this.tslVolFilterCP = obj1.filterCp;
            this.tslHH = obj1.filterHH;
            this.tslHHVip = obj1.filterHHvip;
            ...
            for(let obj in data) {
              ...
              for(let i = 0; i < data[obj].length; i++){
                ...
                this.tslgdCD += parseFloat(userData.tklgd);
                // if(isFilterDate){
                //   this.tslVolFilter += parseFloat(userData.f_bet != null ? userData.f_bet :0);
                // }
                ...
                this.treeData[0].children.push({title: `<span style='color: white'>Cấp ${c} - ${data[obj][i].nick_name} <br>- Volume: ${ this.formatPrice(data[obj][i].tklgd, 2)}</span> <br>- Tổng tiền nạp: ${this.formatPrice(data[obj][i].amt, 2)}<br>- Tổng tiền rút: ${this.formatPrice(data[obj][i].rut, 2)} <br>- Tổng tiền thắng: ${this.formatPrice(data[obj][i].priceWin, 2)}  <br>- Tổng tiền thua: ${this.formatPrice(data[obj][i].priceLose, 2)}<br>- Tổng HHVIP: ${this.formatPrice(data[obj][i].commission_vip, 2)}<br>- Tổng HHGD: ${this.formatPrice(data[obj][i].pending_commission, 2)}`})
              }
            }
        }
      })
    ...
    getData(){
      ...
      .then((resp) => {
        ...
        if(resp.data.success == 4){
            ...
            this.productsFake.forEach(element => {
              element.pricePlay = parseFloat(element.acc_amount)
              element.priceLose = parseFloat(element.acc_lose)
              element.priceWin = parseFloat(element.acc_win)
            });
